...
const init = env => {
  ...
  const m = {
    I: ['ID', '123123'],
    U: ['UUID', '26687cd8-fcb8-4189-974c-7513f08fe875'],
    P: ['IP', ['1.1.1.1']],
    T: ['TXT', []],
    R: ['PROXYIP', 'sjc.o00o.ooo'],
    F: ['ENABLE_REVERSE_PROXY', true],
    N: ['NAT64', false],
    N2: ['NODE_NAME', 'kb']
  };
  ...
  async fetch(req, env) {
    ...
    if (up !== 'websocket') {
      if (url.pathname === `/${cfg.I}`)
        return new Response(`Subscription URL: https://${host}/${cfg.I}/vless`, { status: 200 });
    ...
    try {
      ...
      if (!chk(id)) return new Response('Invalid UUID', { status: 403 });
