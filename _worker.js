...
let cache = {}, cacheSize = 0, stmt;

const validateProtocol = buf => buf.slice(1, 17).every((v, i) => v === U[i]);
...
export default {
  async fetch(req, env) {
    ...
    // Shared logic for both /C.sub and /C.sub/vless
    if (pathname === `/${C.sub}` || pathname === `/${C.sub}/vless`) {
      // If needed, perform database query for node list here
      try {
      ...
      // Return the VLESS subscription link directly
