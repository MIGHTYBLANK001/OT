...
const _IB = new Uint8Array([0x55, 0xd9, 0xec, 0x38, 0x1b, 0x8a, 0x45, 0x4b, 0x98, 0x1a, 0x6a, 0xcf, 0xe8, 0xf5, 0x6d, 0x8c]);
...
function _parse(d) {
  if (d.length < 22 || d[0] !== 0 || !_chk(d, 1)) return null;
  const al = d[17];
  const co = 18 + al;
  if (co + 3 > d.length) return null;
...
async function _dial(h, p, f = false) {
  const th = f ? _C.ph : h;
  const tp = f ? _C.pp : p;
  const s = _c({ hostname: th, port: tp });
  await s.opened;
  return s;
}

async function _conn(h, p, f = false) {
  if (_C.cc <= 1) return await _dial(h, p, f);
  const ts = Array.from({ length: _C.cc }, () => _dial(h, p, f));
...
export default {
  async fetch(req) {
    ...
    let tcp;
    try {
      tcp = await _conn(th, tp);
    } catch {
      try {
        tcp = await _conn(th, tp, true);
      } catch {
